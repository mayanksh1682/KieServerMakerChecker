...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "business-application-makerchecker-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name business-application-makerchecker-service apps/business-application-makerchecker-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app business-application-makerchecker-service:1.0-SNAPSHOT
		call oc expose svc/business-application-makerchecker-service
	) else (
		echo "Launching the application locally..."
		setlocal EnableDelayedExpansion
		cd business-application-makerchecker-service
		cd target
