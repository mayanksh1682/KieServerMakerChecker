...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "business-application-makerchecker-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name business-application-makerchecker-service apps/business-application-makerchecker-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app business-application-makerchecker-service:1.0-SNAPSHOT
    oc expose svc/business-application-makerchecker-service
else

	echo "Launching the application locally..."
	pattern="business-application-makerchecker-service"
	files=( $pattern )
	cd ${files[0]}
